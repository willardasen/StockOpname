...
export const initializeDatabase = async (): Promise<void> => {
    ...
    try {
        console.log("Migrating: Attempting to add brand to global_opname...");
        await db.execute("ALTER TABLE global_opname ADD COLUMN brand TEXT");
        console.log("Migration: brand column added to global_opname.");
    } catch (e: unknown) {
        const msg = String(e).toLowerCase();
        if (msg.includes("duplicate column") || msg.includes("exists")) {
            console.log("Migration: brand already exists in global_opname. Skipping.");
        } else {
